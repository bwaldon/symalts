...
var symPre = ["Some of the symbols are", "There are four", "There is a", ["Some of the symbols are", "but not all of them are"], ["There are four", "but there are no more than four"], ["There is a", "but there is nothing besides a"]]
...
function buildTrials() {
  ...
  for (let n = 0; n < 4; n++) { // number of each
    for (let t = 0; t < 3; t++) { // response (expression): 0(some), 1(four), 2(adhoc)
      for (let s = 0; s < 4; s++) { // strength: 0(weak), 1(strong), 2(exhaustive), 3(baseline)
          dict = {};
...
function specifyTrialCards(trialDict) {
  ...
  } else if (strength == 2){ // if exhaustive (note: same as strong)
    if (primeCat == 0) {
      primeOne[primeOne.indexOf(0)] = someWeak
      primeOne[primeOne.indexOf(1)] = someStrong
      primeTwo[primeTwo.indexOf(0)] = someWeak
...
function conditionSentence(condition, condition_2, symbols, triplet) { // condition --> prime, condition_2 --> strength
  ...
  if (condition_2 == 2) { // prime_type == exhaustive
    if (triplet !== 2) {
      if (condition == 0 | condition == 1) // some or number
        condText = "" + symPre[condition+3][0] + " " + symText[symbols[0]] + "s" + ", " + symPre[condition+3][1] + " " + symText[symbols[0]] + "s" + "."
      if (condition == 2) // adhoc
        condText = "" + symPre[condition+3][0] + " " + symText[symbols[0]] + ", " + symPre[condition+3][1] + " " + symText[symbols[0]] + "."// Brandon mod 
    }
  }
